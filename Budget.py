...
class MainWindow:
    def __init__(self):
        ...
        self.root.geometry("900x500") # Set dimension size
        self.root.resizable(width=False, height=False) # Lock app dimension
        self.root.title("Budget app") # Window title
...
##########CREATING WIDGETS################
    ...
    def draw(self, msg):
        ...
        self.textBox = tk.Text(self.frameText, height=19, width=50, relief='flat', bg='black', font=self.style)
    ...
    def buttons(self):
        self.frame = tk.Frame(self.root, height=2, width=50)
        self.frame.grid(row=0, column=1)
        self.label = tk.Label(self.frame, text='Your Budget:')
        self.entry = tk.Entry(self.frame, font=12)
        self.calBudget = tk.Button(self.frame, text='spending budget', command=self.popUp, width=20)
        self.viewBudget = tk.Button(self.frame, text='Budget Plan', command=self.viewBudgetPlan, width=20)
        self.viewSpending = tk.Button(self.frame, text='calculate Budget', command=self.calculateBudget, width=20)
